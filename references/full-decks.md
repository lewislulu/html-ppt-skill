...
## 1. xhs-white-editorial — 화이트 매거진 스타일

- **Source inspiration:** `20260409 업그레이드업그레이드버전지식한글저장소/샤오홍슈이미지한국어/v2-흰하단버전/slide_01_cover.html` + `20260412-AI테스트테스트한글안전안전/html/xhs-ai-testing-safety-v2.html`
...
## 2. graphify-dark-graph — 어두운 지식 그래프
...
## 4. hermes-cyber-terminal — 다크 터미널 정직한 리뷰
...
## 5. 흑요석-클로드-그라디언트 — GitHub 진한 보라색 그라데이션
...
## 7. xhs-pastel-card — 소프트 마카롱 슬로우 라이프

- **Source inspiration:** `20260412-obsidian-skills/html/xhs-obsidian-skills.html` + pastel patterns shared with `20260409` v2-흰색 배경 버전
...
## 8. dir-key-nav-minimal — 8색 미니멀 방향 키
...
| 11 | `tech-sharing`     | 8 | GitHub-dark, JetBrains Mono, 터미널 코드 블록, 의제 + Q&A | 기술공유, 사내 테크톡, 컨퍼런스톡 |
| 12 | `weekly-report`    | 7 | 기업 명확성, 8셀 KPI 그리드, 배송 목록, 8주 막대 차트, 다음 주 테이블 | 주간보고서, 팀 현황 업데이트, 사업 검토 |
| 13 | `xhs-post`         | 9 | **3:4 @ 810×1080**, 따뜻한 파스텔, 점선 스티커 카드, 페이지 도트 | Xiaohongshu 그래픽 포스트, Instagram 캐러셀 |
| 14 | `course-module`    | 7 | 따뜻한 종이 + 플레이페어 세리프, 학습 목표의 지속적인 왼쪽 사이드바, MCQ 자가 점검 | 교육 모듈, 온라인 강좌, 워크숍 모듈 |
| 15 | `presenter-mode-reveal` 🎤 | 6 | **스피커 모드만** · tokyo-night 기본값 · 5개 주제 T 키 전환 · 150~300 단어로 구성된 각 페이지 축어 스크립트 예 | **기술공유/강의/강의** - S키를 눌러야 스크립트를 볼 수 있는 장면 ✨ |
...
> 🎤 **모든 연설 시나리오에 권장됨(기술 공유/강좌/로드쇼) `presenter-mode-reveal`** 또는 [presenter-mode.md](./presenter-mode.md) 가이드를 참조하여 추가하세요. `<aside class="notes">` 축어적 원고.
